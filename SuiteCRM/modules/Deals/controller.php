...
class DealsController extends SugarController
{
    ...
    public function action_ApplyChecklistTemplate()
    {
        ...
        $dealId = $db->quote($_REQUEST['deal_id']);
        ...
        // Check permissions
        if (!$template->is_public && $template->created_by != $current_user->id) {
            echo json_encode(array(
                'success' => false,
        ...
        try {
            // Apply template
            $checklist = $template->applyToDeal($dealId);
            
            // Update deal to track checklist
            $sql = "INSERT INTO deals_checklist_templates (id, deal_id, template_id, date_modified)
                    VALUES ('" . create_guid() . "', '{$dealId}', '{$templateId}', NOW())
            ...
            echo json_encode(array(
                'success' => false,
                'message' => 'Error creating checklist: ' . $e->getMessage()
            ));
        }
    ...
    public function action_ExportChecklist()
    {
        ...
        if (!$checklist || $checklist->deleted) {
            sugar_die('Checklist not found');
        }
        
        if ($format == 'excel') {
            $checklist->exportToExcel();
        } else {
            $checklist->exportToPDF();
        }
    }
